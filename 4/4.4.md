...
| `-n`, `--templatename` | `jp2validate` | Name of previously generated configuration file |
| `-s`, `--source` | `{tifpath}` | Path to the media directory for the process |
| `-d`, `--destination` | `{imagepath}` | Path to image root directory for the process |
